...
import (
	...
	"mafia/log/internal/auth"
	config "mafia/log/internal/config"
	"mafia/log/internal/log"
...
func setupTest(t *testing.T, fn func(*Config)) (api.LogClient, api.LogClient, *Config, func()) {
	...
	//clientCredits := credentials.NewTLS(clientTLSConfig)
	//cc, err := grpc.Dial(
	//	l.Addr().String(),
	//	grpc.WithTransportCredentials(clientCredits),
	//)
	//require.NoError(t, err)
	...
	serverCredits := credentials.NewTLS(serverTLSConfig)
	...
	server, err := NewGRPCServer(cfg, grpc.Creds(serverCredits))
	require.NoError(t, err)
...
func testProduceConsume(t *testing.T, client api.LogClient, _ api.LogClient, _ *Config) {
